/**
  This file was derived from
    https://raw.githubusercontent.com/yunjie-yang/TuneMC/master/pythia_space/pythia_gen.cc
    and
    https://raw.githubusercontent.com/yunjie-yang/TuneMC/master/pythia_space/pythia_utils.h

  which are a part of TuneMC framework:
...
void Output_Hadron_Rates(int *rates, int Nbins, float * buffer) {
  for (int iBin = 0 ; iBin < Nbins ; iBin++) {
    buffer[iBin] = rates[iBin];
  }
}

void tune_mc_features(Pythia * pythia, float * buffer) {
  ...
  Thrust thr(2);
  double Thrusts = -12345.0;

  Sphericity sph(1.,2);

  double C_param = -12345.0;
  double D_param = -12345.0;
  ...
  // This code is to inspect to find out feature names;
  ...
  /***********   Output Hadron Rates  *****************/
  /****************************************************/
  Output_Hadron_Rates(mesons, 10, &buffer[feature_index]);
  feature_index += 10;

  Output_Hadron_Rates(baryons, 9, &buffer[feature_index]);
  feature_index += 9;

  Output_Hadron_Rates(charms, 9, &buffer[feature_index]);
  feature_index += 9;

  Output_Hadron_Rates(beauties, 8, &buffer[feature_index]);
  feature_index += 8;
  buffer[feature_index] = nCh_total;
}
